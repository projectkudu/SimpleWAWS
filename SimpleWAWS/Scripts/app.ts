angular.module("tryApp", ["ui.router", "angular.filter"])
    .controller("appController", ["$scope", "$http", "$timeout", "$rootScope", "$state", "$location", "staticDataFactory", ($scope: IAppControllerScope, $http: ng.IHttpService, $timeout: ng.ITimeoutService, $rootScope: ITryRootScope, $state: ng.ui.IStateService, $location: ng.ILocationService, staticDataFactory: IStaticDataFactory) => {
